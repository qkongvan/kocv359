          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-sm text-orange-900 leading-relaxed">
                Phù hợp làm video dạng chuyên gia, giới thiệu dịch vụ, chia sẻ kiến thức, kể chuyện, talking head...
                <br />Có thể tải lên 1 video mẫu hoặc 1 bài viết/kịch bản gốc để AI phân tích và tạo ra kịch bản mới.
                <br />Nễu tải lên ảnh khuôn mặt mẫu có thể mô tả trang phục bằng text hoặc tải lên ảnh nhân vât & trang phục sau đó tách trang phục.
                <br />Lưu ý: lựa chọn phong cách kịch bản + xưng hô + giới tính trước khi tạo kịch bản POV.
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div><label className="block text-[10px] font-black text-slate-500 uppercase mb-2">2. Ảnh khuôn mặt mẫu (Tùy chọn):</label>
                    <div 
                        ...
                        className="h-28 border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center bg-slate-50 cursor-pointer hover:border-orange-400 transition-all overflow-hidden relative group"
                    >
                        {state.facePreviewUrl ? <><img src={state.facePreviewUrl} className="h-full object-contain" /><button onClick={handleRemoveFace} className="absolute top-2 right-2 bg-black/60 text-white w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors z-10"><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg></button></> : <div className="text-center opacity-30"><svg className="w-6 h-6 mx-auto mb-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg><span>Mặt mẫu</span></div>}
                    </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                        ...
                        <div className="md:col-span-3 space-y-2">
                            ...
                            <div 
                                ...
                                className="aspect-[3/4] border-2 border-dashed border-slate-200 rounded-2xl flex items-center justify-center bg-slate-50 cursor-pointer hover:border-orange-400 transition-all overflow-hidden relative group"
                            >
                                {state.processedOutfitUrl || state.outfitPreviewUrl ? (
                                    <>
                                        ...
                                        <button onClick={handleRemoveOutfit} className="absolute top-2 right-2 bg-black/60 text-white w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors z-10">
                            ...
                            {(state.outfitFile || state.outfitPreviewUrl?.startsWith('data:')) && (
                                <button onClick={handleExtractOutfit} disabled={state.isExtractingOutfit} className="w-full py-2 bg-blue-600 text-white text-[9px] font-black rounded-lg hover:bg-blue-700 uppercase disabled:opacity-50 tracking-tighter">
                            ...
                            >
                                {state.backgroundPreviewUrl ? (
                                    <>
                                        ...
                                        <button onClick={handleRemoveBackground} className="absolute top-2 right-2 bg-black/60 text-white w-6 h-6 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors z-10">
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M6 18L18 6M6 6l12 12" /></svg>
                                        </button>
                                    </>
                                ) : (
                        ...
                        <div className="md:col-span-6 space-y-2 flex flex-col">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <button 
                                    ...
                                    className="text-[9px] font-black px-3 py-1.5 bg-slate-100 text-slate-500 rounded-lg border border-slate-200 hover:bg-slate-200 disabled:opacity-50 transition-all uppercase"
                                >
                            ...
                            <textarea 
                                ...
                                className="w-full flex-1 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-medium focus:bg-white focus:border-orange-500 outline-none transition-all resize-none leading-relaxed min-h-[180px]" 
                            />
                        </div>
                    </div>
                </div>
                                ...
                                <div className="p-5 flex flex-col flex-1 gap-4">
                                    <div className="space-y-1">
                                        ...
                                        <div 
                                            ...
                                            className="h-20 border-2 border-dashed border-slate-200 rounded-xl flex items-center justify-center bg-slate-50 cursor-pointer hover:border-orange-400 transition-all overflow-hidden relative group"
                                        >
                                            {seg.background?.url ? (
                                                <>
                                                    ...
                                                    <button 
                                                        ...
                                                        className="absolute top-1 right-1 bg-black/60 text-white w-5 h-5 rounded-full flex items-center justify-center hover:bg-red-600 transition-colors z-10"
                                                    >
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between items-center mb-1"><label className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Kịch bản POV:</label><span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${charCount > 180 ? 'bg-red-100 text-red-600' : 'bg-slate-100 text-slate-400'}`}>{charCount} / 180</span></div>
                                        <textarea value={seg.content} onClick={(e) => (e.target as HTMLTextAreaElement).select()} onChange={e => setState((p:any)=>({...p, segments: p.segments.map((s:any)=>s.id===seg.id?{...s, content: e.target.value}:s)}))} className="w-full h-40 p-3 text-xs font-bold text-slate-700 bg-slate-50 border border-slate-100 rounded-xl outline-none resize-none leading-relaxed focus:bg-white focus:border-orange-500 transition-all" />
                                    ...
                                    <div className="mt-auto flex gap-2">
                                        <button onClick={() => handleGenImage(seg.id)} disabled={seg.image?.loading} className="flex-1 py-3 bg-slate-900 text-white text-[9px] font-black rounded-xl hover:bg-black transition-all uppercase tracking-tighter flex items-center justify-center gap-2">
                                            {seg.image?.loading && <div className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin"></div>}
                                            {(seg.image && seg.image.url) ? "Tạo lại" : "Tạo Ảnh"}
                                        </button>
                                        <button onClick={() => handleGenPrompt(seg.id)} disabled={seg.videoPrompt?.loading} className="flex-1 py-3 bg-orange-50 text-orange-700 text-[9px] font-black rounded-xl border border-orange-100 hover:bg-orange-100 transition-all uppercase tracking-tighter flex items-center justify-center gap-2">
                                            {seg.videoPrompt?.loading && <div className="w-3 h-3 border-2 border-orange-600 border-t-transparent rounded-full animate-spin"></div>}
                                            Prompt
                                        </button>
                                    </div>
                                </div>
                                {(seg.videoPrompt && seg.videoPrompt.visible) && (
                                    <div className="p-4 bg-slate-900 border-t border-slate-800 animate-slideUp">
                                        <div className="flex justify-between items-center mb-2">
                                          <span className="text-[8px] text-orange-500 font-bold uppercase tracking-widest">VEO-3 Prompt</span>
                                          <button 
                ...
                <div className="flex flex-col items-center gap-12 py-12">
                    <div className="flex flex-col md:flex-row gap-4 w-full justify-center px-4">
                        <button 
                            ...
                            className="w-full md:w-auto px-12 py-5 bg-orange-600 text-white font-black rounded-2xl shadow-2xl hover:scale-105 active:scale-95 transition-all text-lg flex items-center justify-center gap-4 uppercase tracking-widest disabled:opacity-50"
                        >
                            {isBulkImageLoading ? (
                        ...
                        <button 
                            ...
                            className="w-full md:w-auto px-12 py-5 bg-slate-900 text-white font-black rounded-2xl shadow-2xl hover:scale-105 active:scale-95 transition-all text-lg flex items-center justify-center gap-4 uppercase tracking-widest disabled:opacity-50"
                        >
                            {isBulkPromptLoading ? (
                              <div className="w-6 h-6 border-4 border-white/30 border-t-white rounded-full animate-spin" />
                            ) : (
