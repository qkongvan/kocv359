...
const TimelapseModule: React.FC = () => {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      const { script, context: analyzedContext } = await service.generateTimelapseScript(basePart, finalPart, sceneCount, progressSteps);
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-orange-900 leading-relaxed">
              Tạo video timelapse chuyên nghiệp mô phỏng quá trình thi công nội thất từ hiện trạng thô đến kết quả hoàn thiện. Công cụ tự động phân tích bối cảnh để tạo ra các khung hình trung gian logic, giúp truyền tải thông điệp về sự thay đổi ngoạn mục và chất lượng công trình.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row gap-10">
          <div className="flex-1 space-y-6">
            <div className="flex justify-between items-center px-1">
                ...
                {context && (
                   <span className="text-[10px] font-black text-blue-600 bg-blue-50 px-3 py-1 rounded-full uppercase">Loại dự án: {context.processType}</span>
                )}
            </div>
          ...
          <div className="flex-1 space-y-8">
            ...
            <button onClick={handleStart} disabled={isProcessing || !basePreview || !finalPreview} className="w-full py-6 bg-slate-900 text-white font-black rounded-2xl shadow-xl hover:bg-black transition-all disabled:opacity-50 uppercase tracking-[0.2em] flex items-center justify-center gap-4">
              {isProcessing ? (
                <>
            ...
            {context && (
              <div className="p-4 bg-blue-50 border border-blue-100 rounded-2xl animate-fadeIn">
                 <h4 className="text-[10px] font-black text-blue-700 uppercase tracking-widest mb-2">Phân tích dự án:</h4>
                 <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[8px] text-blue-400 font-bold uppercase">Chủ thể</p>
                      <p className="text-[10px] font-bold text-blue-900">{context.subject}</p>
                    </div>
                    <div>
                      <p className="text-[8px] text-blue-400 font-bold uppercase">Vật liệu thô</p>
                      <p className="text-[10px] font-bold text-blue-900">{context.rawMaterial}</p>
                    </div>
                 </div>
              </div>
            )}
        ...
        <div className="space-y-12 pb-32">
          <div className="bg-slate-900 p-8 rounded-3xl border border-slate-800 shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-wrap gap-4">
              <button 
                ...
                className="px-6 py-4 bg-orange-600 text-white text-[11px] font-black rounded-xl hover:bg-orange-700 transition-all uppercase flex items-center gap-2 shadow-lg disabled:opacity-50"
              >
                {isBulkImageLoading ? (
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : (
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" clipRule="evenodd" /></svg>
                )}
                Vẽ tất cả ảnh
              </button>
              <button 
                ...
                className="px-6 py-4 bg-slate-700 text-white text-[11px] font-black rounded-xl hover:bg-slate-800 transition-all uppercase flex items-center gap-2 shadow-lg disabled:opacity-50"
              >
                {isBulkPromptLoading ? (
                  <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                ) : (
                  <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path d="M13 10V3L4 14H11V21L20 10H13Z" /></svg>
                )}
                Viết tất cả prompt
              </button>
            </div>
                ...
                <div key={seg.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full group hover:shadow-xl transition-all relative">
                  <div className="relative aspect-[9/16] bg-slate-100 overflow-hidden">
                    {seg.image.url ? (
                      <div className="relative w-full h-full">
                        ...
                        {!seg.image.loading && (
                           <button 
                             ...
                             className="absolute top-4 right-4 w-10 h-10 bg-red-600/90 text-white rounded-full flex items-center justify-center shadow-xl hover:bg-red-700 transition-all z-10 opacity-0 group-hover:opacity-100 scale-90 hover:scale-100"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center">
                        <button onClick={() => handleGenImage(seg.id)} className="w-40 py-3 bg-[#f3591a] hover:bg-orange-700 text-white rounded-xl text-xs font-black shadow-lg uppercase active:scale-95 transition-all">Vẽ ảnh {progress}%</button>
                      </div>
                    )}
                  ...
                  <div className="p-6 space-y-4 flex flex-col flex-1">
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center px-1">
                        ...
                        <span className={`text-[10px] font-black ${seg.content.length > 180 ? 'text-red-500' : 'text-slate-400'}`}>
                      ...
                      <textarea 
                        ...
                        className={`w-full h-24 p-4 bg-slate-50 border ${seg.content.length > 180 ? 'border-red-500' : 'border-slate-200'} rounded-2xl text-xs font-bold text-slate-700 outline-none focus:bg-white resize-none shadow-inner leading-relaxed`} 
                      />
                      {seg.content.length > 180 && (
                        <p className="text-[9px] font-bold text-red-500 uppercase px-1">Tối đa 180 ký tự</p>
                      )}
                    </div>
                    <div className="mt-auto grid grid-cols-2 gap-2">
                      <button 
                        ...
                        className="py-4 bg-slate-900 text-white text-[10px] font-black rounded-xl uppercase transition-all hover:bg-black active:scale-95 shadow-md disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                      ...
                      <button 
                        ...
                        className="py-4 bg-orange-50 text-orange-700 text-[10px] font-black rounded-xl uppercase transition-all border border-orange-100 hover:bg-orange-100 active:scale-95 shadow-sm disabled:opacity-50 flex items-center justify-center gap-2"
                      >
                  ...
                  {seg.videoPrompt.visible && (
                    <div className="p-5 bg-slate-900 border-t border-slate-800 animate-slideUp">
                      ...
                      <p className="text-[10px] text-slate-400 italic leading-relaxed line-clamp-4">"{seg.videoPrompt.text}"</p>
                    </div>
                  )}
                </div>
              );
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 py-12 border-t border-slate-200">
              ...
              <button onClick={() => {
                ...
                link.click();
              }} className="w-full md:w-auto px-12 py-5 bg-orange-600 text-white font-black rounded-[2rem] shadow-xl hover:bg-orange-700 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-base">Tải bộ Prompt</button>
          </div>
        </div>
      )}
    </div>
