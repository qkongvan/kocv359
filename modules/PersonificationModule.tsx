        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            <p className="text-sm text-orange-900 leading-relaxed">
              Tạo video Review sản phẩm bằng nhân vật nhân hóa. Nên tải lên ảnh nhân vật nhân hóa mẫu trước để AI đồng bộ trong các khung hình.
              <br />Kịch bản nên có độ dài 160-180 ký tự. Tự động điều chỉnh theo Giọng điệu và Cách xưng hô bạn chọn.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div>
              ...
              <textarea 
                ...
                className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-100 outline-none transition-all font-medium text-sm h-40 resize-none leading-relaxed shadow-inner"
              />
            </div>
            <div>
              ...
              <input 
                ...
                className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-100 outline-none font-bold shadow-sm"
              />
            </div>
              ...
              <div>
                ...
                <div className="relative">
                  <input 
                    ...
                    className="w-full p-4 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-100 outline-none font-bold text-sm shadow-sm"
                  />
          ...
          <div className="space-y-6">
              ...
              <div>
                ...
                <select value={state.frameCount} onChange={e => setState((p: any) => ({ ...p, frameCount: parseInt(e.target.value) }))} className="w-full p-4 bg-white border border-slate-200 rounded-2xl font-bold text-orange-600 shadow-sm outline-none">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div 
                  ...
                  className="relative aspect-square border-2 border-dashed border-slate-200 rounded-2xl flex flex-col items-center justify-center bg-slate-50 cursor-pointer hover:border-orange-400 transition-all overflow-hidden shadow-sm"
                >
          ...
          <div className="md:col-span-2 space-y-4">
                  <div className="space-y-2">
                    ...
                    <textarea 
                      ...
                      className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-100 outline-none transition-all font-medium text-xs h-32 resize-none leading-relaxed shadow-inner"
                    />
                  </div>
                  <div className="space-y-2">
                    <button 
                      ...
                      className="w-full py-2 bg-indigo-600 text-white text-[10px] font-black rounded-xl hover:bg-indigo-700 transition-all disabled:opacity-50 flex items-center justify-center gap-2 uppercase tracking-widest shadow-md"
                    >
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center px-1">
                      ...
                      <button 
                        ...
                        className="p-1.5 bg-orange-100 text-orange-600 rounded-lg hover:bg-orange-200 transition-all disabled:opacity-50 shadow-sm"
                    ...
                    <textarea 
                      ...
                      className="w-full p-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-orange-100 outline-none transition-all font-medium text-xs h-24 resize-none leading-relaxed shadow-inner"
                    />
                  </div>
                </div>
              </div>
            ...
            <button onClick={handleGenerateScript} disabled={state.isGeneratingScript || !state.healthKeyword || !state.ctaProduct} className="w-full py-5 bg-slate-900 text-white font-black rounded-2xl shadow-xl hover:bg-black transition-all disabled:opacity-50 flex items-center justify-center gap-3 uppercase tracking-widest">
              {state.isGeneratingScript ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"></div> ĐANG TẠO KỊCH BẢN...</> : "🚀 BẮT ĐẦU TẠO CAMPAIGN"}
            </button>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-12 pb-32">
          <div className="flex flex-col md:flex-row justify-between items-center bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl gap-4">
            <div>
              ...
              <p className="text-[10px] text-orange-500 font-bold uppercase tracking-widest mt-1">{state.voice} • {state.style}</p>
            </div>
            <div className="flex flex-wrap gap-2">
              <button 
                onClick={handleBulkImage} 
                disabled={state.isBulkImageLoading}
                className="px-5 py-2.5 bg-orange-600 text-white text-[10px] font-black rounded-xl hover:bg-orange-700 transition-all uppercase tracking-widest shadow-lg active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {state.isBulkImageLoading && <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                Vẽ tất cả ảnh
              </button>
              <button 
                onClick={handleBulkPrompt} 
                disabled={state.isBulkPromptLoading}
                className="px-5 py-2.5 bg-indigo-600 text-white text-[10px] font-black rounded-xl hover:bg-indigo-700 transition-all uppercase tracking-widest shadow-lg active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {state.isBulkPromptLoading && <div className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                Viết tất cả prompt
              </button>
            </div>
          </div>
            ...
            {state.segments.map((seg: any) => (
              <div key={seg.id} className="bg-white rounded-[2.5rem] border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full group hover:shadow-xl transition-all">
                ...
                <div className="p-6 flex flex-col flex-1 gap-4 bg-white">
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center px-1">
                      <label className="text-[10px] font-black text-orange-600 uppercase tracking-widest">Lời thoại:</label>
                  ...
                  <div className="mt-auto grid grid-cols-2 gap-2 pt-4 border-t border-slate-50">
                    <button 
                      ...
                      className="py-3 bg-slate-900 text-white text-[10px] font-black rounded-xl hover:bg-black transition-all uppercase tracking-tighter disabled:opacity-50 flex items-center justify-center gap-2 shadow-md active:scale-95"
                    >
                    ...
                    <button 
                      ...
                      className="py-3 bg-orange-50 text-orange-700 text-[10px] font-black rounded-xl border border-orange-100 hover:bg-orange-100 transition-all uppercase tracking-tighter disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95"
                    >
                      {seg.videoPrompt.loading ? <div className="w-3 h-3 border-orange-600 border-t-transparent rounded-full animate-spin"></div> : "VEO-3 Prompt"}
                    </button>
                  </div>
                </div>
                ...
                {seg.videoPrompt.visible && (
                  <div className="p-4 bg-slate-900 border-t border-slate-800 animate-slideUp">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[8px] font-black text-orange-500 uppercase tracking-widest">Video Prompt Ready</span>
                      <button 
          ...
          <div className="flex flex-col md:flex-row items-center justify-center gap-6 py-12 border-t border-slate-200">
              <button onClick={downloadAllImages} className="w-full md:w-auto px-12 py-5 bg-slate-900 text-white font-black rounded-[2rem] shadow-xl hover:bg-black transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-base">Tải ZIP Ảnh</button>
              <button onClick={() => { const text = state.segments.map((s: any) => s.videoPrompt.text).filter((t: string) => t).join('\n'); if(!text) return alert("Chưa có prompt nào!"); const blob = new Blob([text], { type: 'text/plain' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `nhan_hoa_prompts_${Date.now()}.txt`; link.click(); }} className="w-full md:w-auto px-12 py-5 bg-orange-600 text-white font-black rounded-[2rem] shadow-xl hover:bg-orange-700 transition-all flex items-center justify-center gap-3 uppercase tracking-widest text-base">Tải bộ Prompt</button>
          </div>
        </div>
      )}
    </div>
