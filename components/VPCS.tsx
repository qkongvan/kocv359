    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      <div className="max-w-4xl mx-auto space-y-10 pb-20">
        <div className="text-center space-y-2">
          <h1 className="text-4xl font-black text-slate-900 tracking-tighter uppercase">Danh mục <span className="text-red-600">VPCS</span></h1>
          <p className="text-sm font-bold text-slate-400 uppercase tracking-widest">Vi Phạm Chính Sách - Tài liệu tham khảo cho AI Content</p>
        </div>

        <div className="grid grid-cols-1 gap-6">
          {sections.map((section, idx) => (
            <div key={idx} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden animate-fadeIn">
              <div className="px-6 py-4 bg-slate-900 flex justify-between items-center">
                <h3 className="text-white font-black text-xs uppercase tracking-widest">{section.title}</h3>
              ...
              <div className="p-6 divide-y divide-slate-100">
                {section.items.map((item, i) => (
                  <div key={i} className="py-4 first:pt-0 last:pb-0 grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <p className="md:col-span-3 text-xs font-bold text-slate-700 leading-relaxed">{item.content}</p>
                  </div>
                ))}
              </div>
            </div>
