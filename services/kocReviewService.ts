...
export const generateKocImagePromptAI = async (
  ...
  const instruction = `
    Nhiệm vụ: Viết một lời nhắc (Prompt) kỹ thuật bằng Tiếng Việt để tạo hình ảnh AI chất lượng cao (8k) cho sản phẩm "${productName}".
    
    !!! QUY TẮC PHÂN TÍCH THAM CHIẾU (BẮT BUỘC) !!!
    1. FACE_REF: Quan sát ảnh mẫu để mô tả chính xác đặc điểm khuôn mặt (mắt, mũi, môi), kiểu tóc và biểu cảm của nhân vật người Việt Nam.
    2. OUTFIT_REF: Mô tả chi tiết trang phục (kiểu dáng, màu sắc, hoa văn, chất liệu vải) từ ảnh mẫu.
    3. PRODUCT_REF: Mô tả cực kỳ chi tiết về "${productName}" (hình dáng, nhãn hiệu, màu sắc, vật liệu, tỷ lệ kích thước sản phẩm so với tỷ lệ nhân vật) để đảm bảo độ nhận diện 100%.
    ...
    - Nhóm trẻ em: child, kid, baby, toddler, minor, underage, teen, schoolgirl, school uniform, lolita. (Thay bằng "adult", "Vietnamese woman", "Vietnamese man").
    ...
    CẤU TRÚC PROMPT:
    - [Style & Lighting]: Professional commercial photography, cinematic lighting, 8k, highly detailed.
    - [Subject Description]: Adult Vietnamese ${gender}, mô tả chi tiết mặt và tóc từ FACE_REF, dáng người cân đối chuyên nghiệp.
    - [Outfit Description]: Chi tiết trang phục từ OUTFIT_REF.
    - [Product Interaction]: Cách nhân vật cầm/sử dụng "${productName}" phù hợp kịch bản và tư thế "${poseLabel}".
    - [Environment]: Bối cảnh bám sát kịch bản và "${backgroundNote}".
    
    YÊU CẦU ĐẦU RA:
    - Trả về DUY NHẤT một đoạn văn bản Tiếng Việt.
    - KHÔNG có tiêu đề, KHÔNG giải thích, KHÔNG xuống dòng.
  `;

  const contents: any[] = [{ text: instruction }];
  if (facePart) {
    contents.push({ text: "FACE REFERENCE (FACE_REF):" });
    contents.push({ inlineData: facePart });
  }
  if (outfitPart) {
    contents.push({ text: "OUTFIT REFERENCE (OUTFIT_REF):" });
    contents.push({ inlineData: outfitPart });
  }
  if (productParts.length > 0) {
    contents.push({ text: "PRODUCT REFERENCES (PRODUCT_REF):" });
