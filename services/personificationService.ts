...
export const generatePersonificationScript = async (
  ...
  const prompt = `
    ...
    3. XƯNG HÔ (BẮT BUỘC): Sử dụng cặp xưng hô "${addressing}" (Người nói - Người nghe) cho các cảnh của Phản diện. Riêng CẢNH CUỐI (CTA) của Sản phẩm phải đổi sang "Tôi - Bạn".
    ...
    5. CẢNH CUỐI CÙNG (CTA & USP): 
       ...
       - XƯNG HÔ TRONG CẢNH NÀY: BẮT BUỘC sử dụng cặp xưng hô "Tôi - Bạn" (Sản phẩm xưng Tôi, người nghe là Bạn).
    ...
    7. QUY TẮC KHỞI ĐẦU & NỘI DUNG (BẮT BUỘC):
       - Cảnh 1 PHẢI bắt đầu bằng câu giới thiệu: "Tao là [Tên nhân vật phản diện] đang ở [Vị trí nhân vật]..." (Sử dụng đại từ xưng hô phù hợp với "${addressing}" nhưng phải giữ cấu trúc "Tôi/Tao là...").
    ...
    - Mỗi đoạn kịch bản BẮT BUỘC có độ dài TỪ 160 ĐẾN 180 KÝ TỰ.
    - Tuyệt đối KHÔNG ĐƯỢC DÀI HƠN 180 ký tự và KHÔNG ĐƯỢC NGẮN HƠN 160 ký tự.
    
    YÊU CẦU NGÔN NGỮ: Tuân thủ quy tắc văn phong vùng miền: ${voiceDetail}.
...
export const generatePersonificationImage = async (
  ...
  const characterBehavior = isFinalStage 
    ...
    : `Nhân vật (Phản diện) tại vị trí mô tả trong "${characterInfo}" phải luôn có biểu cảm CỤC CẰN, GIẬN DỮ, CÁU GẮT, KHÓ CHỊU, và GHÊ RỢN, có mắt, mũi và miệng đang cử động. TUYỆT ĐỐI KHÔNG CÓ TAY, KHÔNG CÓ CHÂN.`;

  const prompt = `
    Phong cách: ${stylePrompt}
    Hướng: DỌC 9:16 TOÀN MÀN HÌNH.
    QUAN TRỌNG: KHÔNG CÓ THANH ĐEN, KHÔNG CÓ HIỆU ỨNG LETTERBOXING. Hình ảnh PHẢI lấp đầy toàn bộ khung hình 9:16.
    CHÍNH SÁCH NGHIÊM NGẶT KHÔNG CÓ CHỮ: TUYỆT ĐỐI KHÔNG CÓ CHỮ CÁI, TỪ NGỮ, SỐ HOẶC KÝ TỰ TRONG HÌNH ẢNH. 
    !!! KHÔNG THÊM BẤT KỲ CHÚ THÍCH, PHỤ ĐỀ HOẶC HỘP THOẠI NÀO !!!
    
    MÔ TẢ NHÂN VẬT & VỊ TRÍ: "${isFinalStage ? 'Sản phẩm ' + ctaProduct : characterInfo}"
    MÔ TẢ BỐI CẢNH CHUNG: "${backgroundDescription}"
    ${characterFidelity}
    ${stageVisualInstruction}
    
    !!! Ý TƯỞNG CẢNH CỤ THỂ: "${sceneIdea}" !!!
    Mô tả hành động: "${script}".
    
    ${characterBehavior}
    LOGIC GIẢI PHẪU & TÍNH THỰC TẾ (ANATOMICAL LOGIC & REALISM): Bối cảnh PHẢI tuân thủ đúng cấu trúc giải phẫu thực tế. Nếu là trong miệng, chỉ có 2 hàm răng (trên và dưới) đối xứng nhau, KHÔNG được có răng mọc lộn xộn, KHÔNG có nhiều hàm răng chồng chéo. Các bộ phận cơ thể phải có hình dạng và vị trí logic.
    TÍCH HỢP MÔI TRƯỜNG (ENVIRONMENTAL INTEGRATION): Nhân vật PHẢI được tích hợp tự nhiên và logic vào bối cảnh tại vị trí được mô tả trong: "${characterInfo}". Nếu nhân vật là một bộ phận cơ thể (như răng, lá gan, phổi), nó PHẢI xuất hiện đúng vị trí giải phẫu bên cạnh các bộ phận cùng loại khác hoặc trong môi trường tự nhiên của nó (ví dụ: chiếc răng nhân hóa phải đứng trong hàm răng cùng các chiếc răng bình thường khác, lá gan phải nằm trong khoang cơ thể). TUYỆT ĐỐI KHÔNG để nhân vật nằm cô lập, lơ lửng hoặc tách biệt khỏi môi trường xung quanh.
    TỶ LỆ HỢP LÝ (PROPORTIONAL SCALING): Kích thước của nhân vật phải nhỏ gọn và phù hợp với tỷ lệ thực tế của bối cảnh xung quanh, tạo cảm giác nó là một phần của thế giới đó.
    
    Bối cảnh: Môi trường PHẢI tuân thủ nghiêm ngặt BỐI CẢNH CHUNG: "${backgroundDescription}". Chi tiết cảnh cụ thể: "${sceneIdea}". Bạn phải tích hợp ngoại hình của nhân vật và phông nền chính xác như được mô tả trong BỐI CẢNH CHUNG.
    
    ${visualRules}
    ${regenNote ? `Phản hồi bổ sung: ${regenNote}` : ""}
  `;
...
export const generatePersonificationVeoPrompt = async (
  ...
  const systemPrompt = `
    ...
    Đoạn 1: Nhân vật & bối cảnh. (Mô tả nhân vật & vị trí: ${isFinalStage ? 'Sản phẩm ' + ctaProduct : characterInfo}. Phong cách: ${stylePrompt}. BỐI CẢNH CHUNG PHẢI TUÂN THỦ TUYỆT ĐỐI: ${backgroundDescription}. Bối cảnh cụ thể: ${sceneIdea}. ${stageVisualInstruction}. LOGIC GIẢI PHẪU: Bối cảnh tuân thủ đúng cấu trúc giải phẫu thực tế, không có cấu trúc kỳ dị, không có nhiều hàm răng chồng chéo. TÍCH HỢP MÔI TRƯỜNG: Nhân vật PHẢI được tích hợp tự nhiên và logic vào bối cảnh tại vị trí mô tả trong: ${characterInfo}. TỶ LỆ HỢP LÝ: Kích thước nhân vật nhỏ gọn, phù hợp với tỷ lệ thực tế của môi trường xung quanh).
